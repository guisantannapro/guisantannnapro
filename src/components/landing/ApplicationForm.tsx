...
const SectionTitle = ({ icon, children }: { icon: string; children: React.ReactNode }) => (
  <h3 className="font-display text-xl uppercase text-primary border-b border-border pb-2 mb-6 mt-10 first:mt-0 flex items-center gap-2">
...
const Field = ({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) => (
  <div className="flex flex-col gap-1.5">
    <label className="text-sm text-muted-foreground font-body normal-case">
...
const inputClass = "w-full bg-muted border border-border rounded-md px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-colors font-body text-sm normal-case";

const RadioGroup = ({ options, value, onChange }: { options: string[]; value: string; onChange: (v: string) => void }) => (
  <div className="flex flex-col gap-2">
    {options.map((opt) => (
      <label key={opt} className="flex items-center gap-3 cursor-pointer text-sm text-foreground font-body normal-case">
...
const ApplicationForm = () => {
  ...
  const buildWhatsAppMessage = () => {
    ...
    const lines: string[] = [
      ...
      "*Consultoria Fitness — Guilherme Sant'Anna*",
      ...
      "",
      "🔹 *OBJETIVO*",
      `Objetivo principal: ${form.mainGoal}${form.mainGoal === "Outro" ? ` (${form.mainGoalOther})` : ""}`,
      ...
      <section id="application-form" className="py-24 bg-background">
        <div className="container mx-auto px-4 max-w-2xl text-center">
          ...
          >
            ...
            <h2 className="font-display text-3xl uppercase mb-4">Formulário Enviado!</h2>
    ...
    <section id="application-form" className="py-24 bg-background">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <motion.form
          ...
          className="bg-card border border-border rounded-lg p-8 md:p-12 shadow-lg"
        >
          ...
          <div className="space-y-5">
            <Field label="Qual seu objetivo principal?" required>
              <RadioGroup options={goalOptions} value={form.mainGoal} onChange={(v) => update("mainGoal", v)} />
            </Field>
            {form.mainGoal === "Outro" && (
          ...
          <div className="space-y-5">
            <Field label="Quais modalidades pratica atualmente?">
              <CheckboxGroup
                ...
                onChange={(v) => update("trainingModalities", v)}
              />
            </Field>
          ...
          <div className="bg-muted rounded-lg p-6 border border-border space-y-3">
            <p className="text-foreground text-sm font-semibold normal-case">Enviar fotos em jejum:</p>
            <ul className="text-muted-foreground text-sm space-y-1 list-disc list-inside normal-case">
              <li>Frente</li>
              <li>Lado</li>
              <li>Costas</li>
            </ul>
            <div className="text-muted-foreground text-sm space-y-1 normal-case">
              <p>✔ Boa iluminação</p>
              <p>✔ Mesmo local</p>
            ...
            <div className="mt-4 p-4 bg-primary/10 border border-primary/30 rounded-lg">
              <p className="text-foreground text-sm font-semibold normal-case">📲 Após enviar este formulário, envie suas fotos separadamente pelo WhatsApp.</p>
          ...
          <div className="mt-10 border-t border-border pt-8">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              />
              <span className="text-sm text-muted-foreground leading-relaxed normal-case">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className="w-full mt-8 bg-gradient-gold font-display uppercase tracking-widest text-primary-foreground px-10 py-4 text-lg font-semibold glow-gold rounded-md"
          >
            Enviar Inscrição
          </motion.button>
        </motion.form>
